//
//  ProductsLocalizationsTests.swift
//  MVCTests
//
...
final class ProductsLocalizationsTests: XCTestCase {
    ...
    func test_localizedString_haveKeysAndValuesForAllSupportedLanguages() {
        ...
        let table = "Products"
        let presentationBundle = Bundle(for: ProductsViewController.self)
        let localizationBundles = allLocalizationBundles(in: presentationBundle) // different bundle for every localization (it/en etc)
        let localizedStringKeys = allLocalizedStringKeys(in: localizationBundles, table: table) // find all the keys
